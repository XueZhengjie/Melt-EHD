...
def RegisterWithTransform(csvDir,imgDir,cornerPointsT,cornerPointsRGB,dstPoint,alignTxtPath):
    
    # 获取当前文件夹下的所有csv文件
    csv_files = [file for file in os.listdir(csvDir) if file.endswith('.csv')]
    # 将csv文件按文件名从小到大排序
    csv_files.sort(key = lambda x: int(x[:-4]))
    
    #间隔csv_interval文件自动取csv
    csv_interval=30
    print("请输入利于分辨对齐的温度图【第几帧】开始,后续会隔"+str(csv_interval)+"个csv文件自动取")
    # a,b=map(int,input().split())
    a=int(input())
    a_csv=str(a)+".csv"
    # 找到a在list中的index
    try:
        index_a = csv_files.index(a_csv)
    except ValueError:
        print(str(a)+".csv不是当前文件夹下的csv文件。")
        exit()
    
    
    for i in range(index_a, len(csv_files), csv_interval):

        a_csv=csv_files[i]
        print("现在取的是"+a_csv)
        a=int(a_csv.split('.')[0])
        csvPath=os.path.join(csvDir,a_csv)
        ...
        while True:
            for filename in os.listdir(imgDir):
                # Check if the file is a jpg and within the specified range
                start,end=a-25, a  #相机后启动，所以不是对称区间
                if filename.endswith('.jpg') and int(filename.split('.')[0]) in range(start,end):
                    # Read the image file
                    img = cv_imread(os.path.join(imgDir, filename))
                    img=WarpPerspectiveImg(img,cornerPointsRGB,dstPoint)
                    #混合在一起
                    overlapping = cv2.addWeighted(imgt, 0.7, img, 0.3, 0)
                    # Display the image
                    cv2.imshow(a_csv+"remenber the number,press ENTER to finish:"+filename, overlapping)
                    # Wait for a key press and close the window
                    key = cv2.waitKey(0) & 0xFF
                    if key == 13: # ENTER key pressed
                        # best_number.append(int(filename.split('.')[0]))
                        best_number=int(filename.split('.')[0])
            ...
            if best_number is not None:
                break        
        print('刚才结束时候的img数字是：',str(best_number))
        print("请输入刚才混合图的最佳img数字：")
        # a,b=map(int,input().split())
        b=int(input())
        # offset=b-a
        offset=a-b
        note=a_csv+"帧时,对应相机图片"+str(b)+".jpg,时间间隔"+a_csv.split('.')[0]+"-"+str(b)+"="+str(offset)+",即"+a_csv.split('.')[0]+"帧温度图对应"+str(b)+"帧相机图最合适"
        print(note)
        with open(alignTxtPath, 'a') as fd:
            fd.writelines(note)
            fd.writelines("\n")      
...
alignTxtPath=configs['alignTxtPath']
...
MkDir([imgDir,csvDir,tempDir,savePointsDir])

#取温度图和rgb图的透视变化标定点
cornerPointsT,cornerPointsRGB=ReadCornerPoints(cornerPath)
#对当前工况，对齐时间，即对齐温度图帧和rgb图帧
RegisterWithTransform(csvDir,imgDir,cornerPointsT,cornerPointsRGB,dstPoint,alignTxtPath)
# txtSaver=[]
# for i, csvFile in enumerate(csvList):
#     if i % x == 0:  # 每隔x个文件取一个文件
#         #取一个csv和img
#         imgFile=imgList[i]
#         #读取文件img和imgt
#         img=cv_imread(os.path.join(imgDir, imgFile))
#         # imgt=csv_imread(os.path.join(csvDir, csvFile)) #目前没用到
#         #根据角点得到变换后的img和imgt
#         img=WarpPerspectiveImg(img,cornerPointsRGB,dstPoint)
#         # imgt=WarpPerspectiveImg(imgt,cornerPointsT,dstPoint)#目前没用到
#         #rgb图保存到temp作为中转+便于可视化
#         tempFile=imgFile
#         tempPath=os.path.join(tempDir, tempFile)
#         cv2.imencode('.jpg', img)[1].tofile(tempPath)  # 输出图像
#         #取图片的交点
#         Points,mergedPoints=GetIntersection(tempPath)
#         # 让我康康机器提取的点是啥样
#         image=ShowIntersectionImg(tempPath,Points,mergedPoints,False,None)
#         #多边形框选出要的点的范围并剔除多边形外的点
#         #目前网格是18x18，框选剔除最边上两条，故余下点14x14=196
#         #手动删补点
#         innerPoints=BoxSelectPoints(image,mergedPoints,n,None)
#         finPoints=NumberPoints(image,innerPoints,n,None)
#         #取每个点的温度值啦
#         csvPath=os.path.join(csvDir, csvFile)
#         pointsT=GetCSVTemperature(csvPath,img,dstPoint,cornerPointsT,finPoints,savePath=None)
#         pointsFile=str(i)+".csv"
#         savePointsPath=os.path.join(savePointsDir, pointsFile)
#         SavePoints(pointsT,savePointsPath)
#         txtSaver.append([csvFile,imgFile,pointsFile])

# with open(txtPath, 'w') as fd:
#     for d in txtSaver:
#         fd.writelines(str(d))
#         fd.writelines("\n")    
# print("##################记录已保存到："+txtPath)
# print("###############当前工况已完成################")
