...
csvList,imgList=RegisterWithTransform2(csvDir,imgDir,cornerPointsT,cornerPointsRGB,dstPoint)
...
if os.path.exists(txtPath):
    with open(txtPath, 'r') as f:
        last_csv = f.readlines()[-1].split()[0]
        if last_csv in csvList:
            start_index = csvList.index(last_csv)+1
        else:
            start_index = 0
else:
    start_index = 0
    
for i, csvFile in enumerate(csvList[start_index:]):
    if i % x == 0:  # 每隔x个文件取一个文件
        #取一个csv和img
        imgFile=imgList[i+start_index]
        print("当前执行："+csvFile+" with "+imgFile)
        #读取文件img和imgt
        ...
        #手动删补点
        print("执行中："+csvFile+" with "+imgFile)
        print("【指定多边形包围框，增删点】")
